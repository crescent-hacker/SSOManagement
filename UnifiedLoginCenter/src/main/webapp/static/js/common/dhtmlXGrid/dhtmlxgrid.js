...
var globalActiveDHTMLGridObject;String.prototype._dhx_trim=function(){return this.replace(/&nbsp;/g, " ").replace(/(^[ \t]*)|([ \t]*$)/g, "");};function dhtmlxArray(ar){return dhtmlXHeir((ar||new Array()), dhtmlxArray._master);};dhtmlxArray._master={_dhx_find:function(pattern){for (var i = 0;i < this.length;i++){if (pattern == this[i])return i;};return -1;},
 ...
 this.fldSorted=el;this.r_fldSorted=r_el;var c = this.hdr.rows[1];var c = r_el.parentNode;var real_el = c._childIndexes ? c._childIndexes[el._cellIndex] : el._cellIndex;this.setSortImgPos(false, false, false, r_el);}};this.enableHeaderImages=function(fl){this.useImagesInHeader=fl;};this.setHeaderNew=function(hdrStr, splitSign, styles){
	 if (typeof (hdrStr) != "object")
		 ...
		 var alCenter=this._eSplit(hdrStr);
 ...
 return true;var r = c.parentNode;if (!r.idd||r.idd == "__filler__")return;var el = (e ? e.target : event.srcElement);if (r.idd == window.unknown)return true;if(c.innerHTML.indexOf("<img")>-1||c.innerHTML.indexOf("<IMG")>-1)return true;if (!this.grid.callEvent("onMouseOver", [
 r.idd,
 c._cellIndex,
 (e||window.event)]))
 ...
 attachHeaderNew:function(values, style, _type){
	 var alCenter=new Array();
	 var hasStyle=false;
		...
		for (var i = 0; i < values.length; i++){
			var newstyle="";
			if (hasStyle)newstyle=style[i];
			alCenter[i]="text-align:center;"+newstyle;
		}
