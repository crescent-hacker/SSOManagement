...
public class IPPropertyUtil {
     ...
     * 日志记录器
     */
    private static Logger logger = LoggerFactory.getLogger(com.jetsun.remotelogin.utility.property.IPPropertyUtil.class);
    /**
     * 配置对象
     */
...
//            InputStream input = IPPropertyUtil.class.getClassLoader().getResourceAsStream(ipPropertyName);
            InputStream input = new FileInputStream(com.jetsun.remotelogin.utility.property.IPPropertyUtil.class.getResource(ipPropertyName).getFile());
            BufferedReader bf = new BufferedReader(new InputStreamReader(input, "utf-8"));
            config = new Properties();
            config.load(bf);
            readAllProperties();
            bf.close();
            realFilePath = com.jetsun.remotelogin.utility.property.IPPropertyUtil.class.getClassLoader().getResource(ipPropertyName).getFile();
