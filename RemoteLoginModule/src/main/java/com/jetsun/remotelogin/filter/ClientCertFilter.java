...
public class ClientCertFilter implements Filter {
    private Logger logger = LoggerFactory.getLogger(com.jetsun.remotelogin.filter.ClientCertFilter.class);
    ...
    public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
        ...
        if (certChain == null || certChain.length == 0) {
            logger.debug("current user hasn't commit any certificate.");
        } else {//证书存在
            try {
                ...
                LoginHandler loginHandler = (LoginHandler) ctx.getBean("loginHandler");//dao初始化

                //查库检查证书是否存在和关联了用户
                isCertValid = loginHandler.isCertExist(cert.getSerialNumber().toString(),session);

            } catch (CertificateExpiredException e) {
                //证书已过期
                logger.error("certificate has been out-dated", e);
            } catch (CertificateNotYetValidException e) {
                //证书未生效
                logger.error("certificate has not valid yet.", e);
            } finally {
